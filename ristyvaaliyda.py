...
def write_ristyva_inside_circle(text_color, x, y):
    my_turtle_cursor.penup()
    my_turtle_cursor.color(text_color)
    my_turtle_cursor.goto(x, y)
    my_turtle_cursor.begin_fill()
    my_turtle_cursor.pendown()
    my_turtle_cursor.write("Ristyva", font=("sans-serif", 26, "bold"))
    my_turtle_cursor.getscreen().update()

def write_aliyda_inside_circle(text_color, x, y):
    my_turtle_cursor.penup()
    my_turtle_cursor.color(text_color)
    my_turtle_cursor.goto(x, y)
    my_turtle_cursor.begin_fill()
    my_turtle_cursor.pendown()
    my_turtle_cursor.write("Aliyda", font=("sans-serif", 26, "bold"))
    my_turtle_cursor.getscreen().update()
...
def write_happy_birthday(text_color, x, y):
    my_turtle_cursor.penup()
    my_turtle_cursor.color(text_color)
    my_turtle_cursor.goto(x, y)
    my_turtle_cursor.pendown()
    my_turtle_cursor.write("Happy Birthday!", font=("sans-serif", 24, "bold"))
    my_turtle_cursor.getscreen().update()

# Background color for the cake
my_turtle_screen.bgcolor("#FFFDD0")

# List of layers for the cake
parts_of_cake = []
parts_of_cake.append(["#A020F0", "#000000", 3, 30])
parts_of_cake.append(["#55FF55", "#000000", 3, 20])
parts_of_cake.append(["#B87333", "#000000", 3, 60])

# Draw the bottom layer of the cake
draw_layer_of_the_cake("#FFC0CB", "#000000", 3, -220, y_coordinate - 70, 400, 10)

# Draw each layer of the cake
...
# Draw the circle on top of the candle
draw_circle_on_top_of_stick("#181A18", "#FFFDD0", 100, y_coordinate + 235, 100)

# Write names inside the circles
write_ristyva_inside_circle("#000000", -70, y_coordinate + 240)
write_aliyda_inside_circle("#000000", -80, y_coordinate + 190)

# Write "Happy Birthday" below the cake
write_happy_birthday("#000000", -100, y_coordinate - 240)
